...
// akira modified start
	...
	//	$validate	= new CValidate();
		if( function_exists( 'config_sanitize_class' ) )
		{
			$sanitizename	= config_sanitize_class();
			$sanitize		= new $sanitizename();
		}
		else
		{
			$sanitize	= new CSanitize();
		}
		if( function_exists( 'config_validate_class' ) )
		{
			$validatename	= config_validate_class();
